...
from structMDC_model import StructMDCModel
...
def train(train_image_path,
          ...
          # Mesh Depth Refine settings
          n_convolution_mesh_depth_refine=settings.N_CONVOLUTION_MESH_DEPTH_REFINE,
          n_filter_mesh_depth_refine=settings.N_FILTER_MESH_DEPTH_REFINE,
    ...
    depth_model_checkpoint_path_best = os.path.join(checkpoint_path, 'depth_model-best.pth')
    pose_model_checkpoint_path_best = os.path.join(checkpoint_path, 'pose_model-best.pth')
    ...
    train_mesh_depth_paths = data_utils.read_paths(train_mesh_depth_path)
    train_validitiy_map_paths = data_utils.read_paths(train_validity_map_path)
    train_intrinsics_paths = data_utils.read_paths(train_intrinsics_path)

    n_train_sample = len(train_image_paths)

    assert len(train_mesh_depth_paths) == n_train_sample
    assert len(train_validitiy_map_paths) == n_train_sample
    ...
    train_dataloader = torch.utils.data.DataLoader(
        datasets.KBNetTrainingDataset(
            ...
            sparse_depth_paths=train_mesh_depth_paths,
            validity_map_paths=train_validitiy_map_paths,
            intrinsics_paths=train_intrinsics_paths,
            shape=(n_height, n_width),
            cropped_margin=cropped_margin,
    ...
    validation_available = val_image_path is not None and \
        val_mesh_depth_path is not None and \
    ...
    if validation_available:
        ...
        val_mesh_depth_paths = data_utils.read_paths(val_mesh_depth_path)
        ...
        assert len(val_mesh_depth_paths) == n_val_sample
        assert len(val_validity_map_paths) == n_val_sample
        ...
        val_dataloader = torch.utils.data.DataLoader(
            datasets.KBNetInferenceDataset(
                ...
                sparse_depth_paths=val_mesh_depth_paths,
    ...
    depth_model = StructMDCModel(
        input_channels_image=input_channels_image,
        input_channels_depth=input_channels_depth,
        n_convolution_mesh_depth_refine=n_convolution_mesh_depth_refine,
        n_filter_mesh_depth_refine=n_filter_mesh_depth_refine,
    ...
    for epoch in range(1, learning_schedule[-1] + 1):
        ...
        for inputs in train_dataloader:
            ...
            image0, image1, image2, sparse_depth0, validity_map_depth0, intrinsics = inputs
            ...
            output_depth0, _ = depth_model.forward(
                image=image0,
                mesh_depth=sparse_depth0,
...
def validate(depth_model,
    ...
    delta_105 = np.zeros(n_sample)
    delta_110 = np.zeros(n_sample)
    delta_125 = np.zeros(n_sample)
    delta_1252 = np.zeros(n_sample)
    delta_1253 = np.zeros(n_sample)
    ...
    depths = []
    sparse_depths = []
    indices = np.random.randint(low=0, high=len(dataloader)-1, size=2)
    ...
    for idx, (inputs, ground_truth) in enumerate(zip(dataloader, ground_truths)):
        ...
        image, sparse_depth, validity_map_depth, intrinsics = inputs
        ...
        # save depth results
        if visualize and idx in indices:
            depths.append(output_depth.cpu().detach().numpy())
            # sparse_depths.append(sparse_depth.cpu().detach().numpy() * max_depth)
            sparse_depths.append(sparse_depth.cpu().detach().numpy())
        ...
        delta_err = eval_utils.GetDepthPrintableRatios(ground_truth, output_depth)
        delta_105[idx]  = delta_err['D_1.05']
        delta_110[idx]  = delta_err['D_1.10']
        delta_125[idx]  = delta_err['D_1.25']
        delta_1252[idx] = delta_err['D_1.56']
        delta_1253[idx] = delta_err['D_1.95']
    ...
    delta_105_mean = np.mean(delta_105)
    delta_110_mean = np.mean(delta_110)
    delta_125_mean = np.mean(delta_125)
    delta_1252_mean = np.mean(delta_1252)
    delta_1253_mean = np.mean(delta_1253)
    ...
    log('{:>8}  {:>8}  {:>8}  {:>8}  {:>8}'.format(
            '\u03B4<1.05', '\u03B4<1.10', '\u03B4<1.25', '\u03B4<1.25^2', '\u03B4<1.25^3'),
            log_path)
    log('{:8.3f}  {:8.3f}  {:8.3f}  {:8.3f}  {:8.3f}'.format(
        delta_105_mean, delta_110_mean, delta_125_mean, delta_1252_mean, delta_1253_mean),
        log_path)
    ...
    if np.round(mae, 3) <= np.round(best_results['mae'], 3):
        n_improve = n_improve + 1
        mae_improve = True
    if np.round(rmse, 3) <= np.round(best_results['rmse'], 3):
        n_improve = n_improve + 1
        rmse_improve = True
    ...
    do_save = False
    if mae_improve and rmse_improve:
        ...
        best_results['del105'] = delta_105_mean
        best_results['del110'] = delta_110_mean   
        best_results['del125'] = delta_125_mean   
        best_results['del1252'] = delta_1252_mean   
        best_results['del1253'] = delta_1253_mean    

        do_save = True

    # save depth visulization
    if visualize and do_save:
        depth_path = os.path.join(checkpoint_path, 'depth_' + str(step))
        ...
        if len(depths) > 0:
            for i,d in enumerate(depths):
                ...
                plt.imsave(os.path.join(depth_path, str(i) + '.png'), z, cmap=plt.cm.jet)
            for i,d in enumerate(sparse_depths):
                z = np.uint32(d * 256.0)
                z = np.squeeze(z)
                z = Image.fromarray(z, mode='I')
                plt.imsave(os.path.join(depth_path, str(i) + '_sparse.png'), z, cmap=plt.cm.jet)
    ...
    log('{:8.3f}  {:8.3f}  {:8.3f}  {:8.3f}  {:8.3f}'.format(
        ...
        best_results['del110'],
        best_results['del125'],
        best_results['del1252'],
        best_results['del1253']), log_path)

    return best_results, do_save
...
def run(image_path,
    ...
    validitiy_map_paths = data_utils.read_paths(validitiy_map_path)
    ...
    dataloader = torch.utils.data.DataLoader(
        datasets.KBNetInferenceDataset(
            ...
            validity_map_paths=validitiy_map_paths,
    ...
    for idx, (inputs, ground_truth) in enumerate(zip(dataloader, ground_truths)):
        ...
        if save_outputs:
            ...
            refine_depths.append(np.squeeze(input_depth))
    ...
    if save_outputs:
        ...
        outputs = zip(images, output_depths, sparse_depths, refine_depths, ground_truths)
        ...
        refine_depth_dirpath = os.path.join(output_path, 'refine_depth')
        ...
        for idx, (image, output_depth, sparse_depth, refine_depth, ground_truth) in enumerate(outputs):
            ...
            refine_depth_path = os.path.join(refine_depth_dirpath, filename)
            data_utils.save_depth(refine_depth, refine_depth_path)
...
def log_network_settings(log_path,
                         # Mesh Depth Refinement settings
                         n_convolution_mesh_depth_refine,
                         n_filter_mesh_depth_refine,
                         # Depth network settings
    ...
    log('Mesh Depth Refine settings:', log_path)
    log('n_convolution_mesh_depth_refine={}'.format(n_convolution_mesh_depth_refine),
        log_path)
    log('n_filter_mesh_depth_refine={}'.format(n_filter_mesh_depth_refine),
        log_path)
    log('', log_path)
