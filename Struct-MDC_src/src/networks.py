...
class SparseToDensePool(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for pool, s in zip(self.min_pools, self.min_pool_sizes):
            ...
            print('min pool: ' + str(z_pool.shape))
        ...
        for pool, s in zip(self.max_pools, self.max_pool_sizes):
            ...
            print('max pool: ' + str(z_pool.shape))


        ## 3) Stack max and minpools into pyramid
        pool_pyramid = torch.cat(pool_pyramid, dim=1)
        
        print('stacked: ' + str(pool_pyramid.shape))


        ## 4)  Learn weights for different kernel sizes, and near and far structures
        pool_convs = self.pool_convs(pool_pyramid)
        print('convolved: ' + str(pool_convs.shape))
        
        pool_convs = torch.cat([pool_convs, x], dim=1)
        print('cat: ' + str(pool_convs.shape))
        
        ## 5) final convolution
        pool_convs = self.conv(pool_convs)
        print('convolved: ' + str(pool_convs.shape))
...
class RefineMeshDepth(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 weight_initializer='kaiming_uniform',
                 activation_func='leaky_relu'):
        super(RefineMeshDepth, self).__init__()

        activation_func = net_utils.activation_func(activation_func)
        
        # convolution layer
        self.conv_depth1 = net_utils.Conv2d(
                in_channels=depth_channels,
                out_channels=n_filter,
                kernel_size=n_convolution,
                stride=1,
                weight_initializer=weight_initializer,
                activation_func=activation_func,
                use_batch_norm=False)
        ...
        self.conv_depth2 = net_utils.Conv2d(
        ...
        self.conv_pool1 = net_utils.Conv2d(
                in_channels=n_filter*8,
                out_channels=n_filter*4,
                kernel_size=n_convolution,
                stride=1,
                weight_initializer=weight_initializer,
                activation_func=activation_func,
                use_batch_norm=False)
        self.conv_pool2 = net_utils.Conv2d(
                in_channels=n_filter*4,
                out_channels=n_filter*4,
                kernel_size=n_convolution,
                stride=1,
                weight_initializer=weight_initializer,
                activation_func=activation_func,
                use_batch_norm=False)
        self.conv_pool3 = net_utils.Conv2d(
                in_channels=n_filter*16,
                out_channels=n_filter*8,
                kernel_size=n_convolution,
                stride=1,
                weight_initializer=weight_initializer,
                activation_func=activation_func,
                use_batch_norm=False)
        self.conv_pool4 = net_utils.Conv2d(
                in_channels=n_filter*8,
                out_channels=n_filter*8,
                kernel_size=n_convolution,
                stride=1,
                ...
                activation_func=activation_func,
                use_batch_norm=False)
        self.conv_pool5 = net_utils.Conv2d(
                in_channels=n_filter*8,
                out_channels=n_filter*8,
                kernel_size=n_convolution,
                stride=1,
        ...
        self.conv_last1 = net_utils.Conv2d(
                in_channels=n_filter*2*(1+2+4),
                out_channels=n_filter*2,
                kernel_size=1,
                stride=1,
                weight_initializer=weight_initializer,
                activation_func=activation_func,
                use_batch_norm=False)
        self.conv_last2 = net_utils.Conv2d(
                in_channels=n_filter*2,
                out_channels=n_filter,
                kernel_size=1,
                stride=1,
                ...
                activation_func=activation_func,
                use_batch_norm=False)
        self.conv_last3 = net_utils.Conv2d(
                in_channels=n_filter,
                out_channels=n_filter,
                kernel_size=1,
                stride=1,
        ...
        self.min_pool1 = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.min_pool2 = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
    ...
    def forward(self, depth, image):
        """
        :param depth: B,1,H,W 
        :param image: B,C(3),H,W
        :return: D_dt B,'8',H,W 
        """

        ########## 0) extract outer mask of Depth
        mask_outer = depth
        tmp = mask_outer
        mask_outer = mask_outer/(tmp+1e-6)
        extend_img = torch.cat([image,mask_outer], dim=1) # [B,4,H,W]

        cnv1_d = self.conv_depth1(depth) # [B,8,H,W]
        cnv1_c = self.conv_img(extend_img) # [B,8,H,W]
        cnv1   = torch.cat([cnv1_d,cnv1_c], dim=1) # [B,16,H,W]
        cnv2   = self.conv_depth2(cnv1) # [B,16,H,W]
        hyper1   = torch.cat([cnv1,cnv2], dim=1) # [B,32,H,W]
        
        max_pool1 = self.max_pool1(hyper1) # [B,32,H/2,W/2]
        min_pool1 = -self.min_pool1(torch.where(hyper1 == 0, -999 * torch.ones_like(hyper1), -hyper1)) 
        min_pool1 = torch.where(min_pool1 == 999, torch.zeros_like(min_pool1), min_pool1) # [B,64,H/2,W/2]
        pool1 = torch.cat([max_pool1, min_pool1], dim=1)

        cnv3  = self.conv_pool1(pool1) # [B,32,H/2,W/2]
        cnv4  = self.conv_pool2(cnv3) # [B,32,H/2,W/2]
        B, C, H, W = cnv2.shape
        up1 = torch.nn.functional.interpolate(cnv4, size=(H,W), mode='bilinear') # [B,32,H,W]
        hyper2 = torch.cat([cnv3, cnv4], dim=1)  # [B,64,H/2,W/2]
        
        
        max_pool2 = self.max_pool2(hyper2) # [B,64,H/4,W/4]
        min_pool2 = -self.min_pool2(torch.where(hyper2 == 0, -999 * torch.ones_like(hyper2), -hyper2)) 
        min_pool2 = torch.where(min_pool2 == 999, torch.zeros_like(min_pool2), min_pool2) # [B,64,H/2,W/2]
        pool2 = torch.cat([max_pool2, min_pool2], dim=1)

        cnv5  = self.conv_pool3(pool2) # [B,64,H/4,W/4]
        cnv6  = self.conv_pool4(cnv5) # [B,64,H/4,W/4]
        cnv7  = self.conv_pool5(cnv6) # [B,64,H/4,W/4]
        up2   = torch.nn.functional.interpolate(cnv7, size=(H,W), mode='bilinear') # [B,64,H/4,W/4]
        cat   = torch.cat([cnv2, up1, up2], dim=1)  # [B,112,H,W]
        
        
        cnv8 = self.conv_last1(cat) # [B,16,H,W]
        cnv9 = self.conv_last2(cnv8) # [B,8,H,W]
        out  = self.conv_last3(cnv9)  # [B,8,H,W]
