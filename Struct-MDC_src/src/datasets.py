...
def random_crop(inputs, shape, intrinsics=None, crop_type=['none']):
    ...
    if intrinsics is not None:
        ...
        intrinsics = intrinsics + [[0.0, 0.0, -x_start],
                                   [0.0, 0.0, -y_start],
                                   [0.0, 0.0, 0.0     ]]
        # Adjust intrinsics for undistortion
        margin = 32
        intrinsics = intrinsics + [[0.0, 0.0, -margin],
                                   [0.0, 0.0, -margin],
                                   [0.0, 0.0, 0.0     ]]
...
class KBNetTrainingDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.validity_map_paths = validity_map_paths
        self.intrinsics_paths = intrinsics_paths

        self.shape = shape
        self.cropped_margin = cropped_margin
        self.do_random_crop = \
    ...
    def __getitem__(self, index):
        ...
        # Load validity map
        validity_map0 = load_depth(self.validity_map_paths[index])

        # Load camera intrinsics
        intrinsics = np.load(self.intrinsics_paths[index]).astype(np.float32)
        # reflect side cropped (for undistortion) 
        intrinsics[0,2] -= self.cropped_margin
        intrinsics[1,2] -= self.cropped_margin

        # Crop image, depth and adjust intrinsics
        if self.do_random_crop:
            [image0, image1, image2, sparse_depth0, validity_map0], intrinsics = random_crop(
                inputs=[image0, image1, image2, sparse_depth0, validity_map0],
                shape=self.shape,
                intrinsics=intrinsics,
                crop_type=self.random_crop_type)

        # Convert to float32
        image0, image1, image2, sparse_depth0, validity_map0, intrinsics = [
            T.astype(np.float32)
            for T in [image0, image1, image2, sparse_depth0, validity_map0, intrinsics]
        ]
        

        return image0, image1, image2, sparse_depth0, validity_map0, intrinsics
...
class KBNetInferenceDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load validity map
        validity_map = load_depth(self.validity_map_paths[index])
        ...
        image, sparse_depth, validity_map, intrinsics = [
            T.astype(np.float32)
            for T in [image, sparse_depth, validity_map, intrinsics]
        ]

        return image, sparse_depth, validity_map, intrinsics
